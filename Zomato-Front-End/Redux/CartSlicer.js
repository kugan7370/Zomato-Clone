...
export const getCardItemsFromDb = createAsyncThunk(
  "get/cardItems",
  async () => {
    const token = await getToken();
    try {
...
const CartSlicer = createSlice({
  ...
  reducers: {
    addCarts: (state, action) => {
      //filter
      const isAdded =
        state.cartItems.filter((item) => item.foodId === action.payload.foodId)
          .length > 0;
      if (isAdded) {
        //remove from cart
...
export const { addCarts, incrementItem, decrementItem, clearCart } =
  CartSlicer.actions;
